...
class CustomModel(tf.keras.Model):
    def __init__(self, input_shape, y_scaler, feature_scaler, A_rc_idx, A_cp_idx, categorical_indices):
        ...
        self.shared_layers = [
            tf.keras.layers.Dense(256, activation='swish', kernel_constraint=tf.keras.constraints.max_norm(0.9)),
            tf.keras.layers.Dense(256, activation='swish', kernel_constraint=tf.keras.constraints.max_norm(0.9)),
            tf.keras.layers.Dense(256, activation='swish', kernel_constraint=tf.keras.constraints.max_norm(0.9)),
...
def load_models():
    ...
    # compelete iles
    all_weights = "Full_custom_model_weights_old_setup.weights.h5"
    full_preparing_obj="Full_preprocessing_objects_old_setup.pkl"

    # -selection files
    sel_weights = "FS_custom_model_weights_old_setup.weights.h5"
    sel_preparing_obj = "FS_preprocessing_objects_old_setup.pkl"
    ...
    def load_single_model(weights_path,preparing_obj_path, name):
        for path, fname in [(weights_path, "Weights"), (preparing_obj_path, "Preparing_objects=preparing_objects_old_setup")]:
...
if col1.button("Guide"):
    ...
    col1.markdown("“First, select the performance prediction method. Then, based on the chosen method, enter the required system parameters. Finally, by clicking the prediction button, you can view the results.”", unsafe_allow_html=True)
...
#......................................
A_rc = st.sidebar.number_input("$A_{rc}$ (m²)", 1e-5, 0.1, 0.001, step=1e-6, format="%.6f")
A_cp = st.sidebar.number_input("$A_{cp}$ (m²)", 1e-5, 0.2, 0.001, step=1e-6, format="%.6f")
G = st.sidebar.number_input("G (W/m²)", 0.0, 1500.0, 500.0, step=10.0)
V = st.sidebar.number_input("V (m/s)", 0.0, 10.0, 2.0, step=0.1)
T_a = st.sidebar.number_input("$T_{a}$ (K)", 273.0, 400.0, 293.0)
A = st.sidebar.number_input("Area A (m²)", 1e-8,1e-4, 1e-6, step=1e-7, format="%.9f")
L = st.sidebar.number_input("Length L (m)", 0.000001, 1e-1, 0.001, step=0.000001, format="%.6f")
optical = st.sidebar.number_input("Optical Efficiency", 0.01, 1.0, 0.9, step=0.01)
alpha_cooler = st.sidebar.number_input("$α_{cooler}$", 0.01, 1.0, 0.05, step=0.01)
taw_2 = st.sidebar.number_input("τ₂", 0.01, 1.0, 0.5, step=0.01)
CR = st.sidebar.number_input("CR", 0.1, 100.0, 5.0, step=0.1)
N = st.sidebar.number_input("N", 1.0, 200.0, 127.0, step=1.0)
...
if is_all_features:
    ...
    taw_3 = st.sidebar.number_input("τ₃", 0.0, 1.0, 0.0, step=0.01)
    taw_4 = st.sidebar.number_input("τ₄", 0.01, 1.0, 0.4, step=0.01)
    delta_cp = st.sidebar.number_input("$Δ_{cp}$ (m)", 1e-6, 1.0, 10*1e-4, step=1e-6, format="%.6f")
    delta_cu = st.sidebar.number_input("$Δ_{cu}$ (m)", 1e-6, 1.0, 2*1e-4, step=1e-6, format="%.6f")
    delta_rc = st.sidebar.number_input("$Δ_{rc}$ (m)", 1e-6, 1.0, 1e-3, step=1e-6, format="%.6f")
    k_rc = st.sidebar.number_input("$k_{rc}$ (W/m·K)", 0.1, 20.0, 2.0, step=1.0, format="%.6f")
    delta_Al = st.sidebar.number_input("$Δ_{Al}$ (m)", 1e-6, 1.0, 1e-3, step=1e-6, format="%.6f")
    k_Al = st.sidebar.number_input("$k_{Al}$ (m)(W/m·K)", 50.0, 500.0, 300.0, step=1.0, format="%.6f")
...
def calculate_th():
    ...
    Abs = np.abs
    sqrt = np.sqrt
    result_k=  0.03566222*G*(-3.0763502*L*(material_val+ (alpha_cooler + epsilon_1**(1.2088965/V) + epsilon_3)**2) + alpha_cooler + 0.0507079602572312) + OpticalCRG/(0.03968428*T_a*sqrt(A_rc*(6.9153543 + V/sqrt(A_cp))/A_cp) - 14.407003) + T_a + (epsilon_2*(-0.026225813*A_rc*T_a*epsilon_4/sqrt(A_cp*V) - 1.9661237) - 0.38108942*taw_1/epsilon_2)*(taw_2*(epsilon_1 + taw_2) + taw_4) - 0.16518472 + Abs((A_cp*OpticalCRG/(1.6452948*A_cp/L + 2.26113842440709*G*L*V/A_rc_A_cp**(0.27862516**(17.8434607463491*A_cp))) + L*OpticalCRG + 0.784869095052954*epsilon_2*taw_2*sqrt(taw_4*sqrt(A_cp*epsilon_4/(A*N))))*(-sqrt(A_cp*(0.2150894 - 0.51114422189793*alpha_cooler/A_rc_A_cp)/(A*(-A_rc_A_cp + N + material_val + 2.1702342)*sqrt(Abs(material_val - 4.145207)))) + 0.33994088 + 0.036386017/(-material_val/(sqrt(A_cp/(A*N)) + 0.9541348))**material_val) + 0.2150894/sqrt(Abs(material_val - 4.154451)))
    return result_k
...
if st.button("Prediction ", type="primary"):
    with st.spinner("Calculating..."):
        ...
        if is_all_features or is_feature_selection:
            ...
            with c2:
                st.metric("power density (P)", f"{perf['Power']:.6f} mW")
...
#.................................................
        if is_eq:
            ...
            with colT1:
                st.metric("T_h", f"{T_h_eq:.3f} °C")
                st.metric("T_c", f"{T_c_eq:.3f} °C")

                with st.expander("Kelvin"):
                    st.write(f"Kelvin: {T_h_eq + 273.15:.3f} K")
                    st.write(f"Kelvin: {T_c_eq + 273.15:.3f} K")
            st.markdown("---")
        ...
        if is_eq:
            with st.expander("Equations"):
                if is_eq:
                    ...
                    \begin{aligned}
                    T_c &= 0.03566\,G \Bigl[ 
                     -3.076\,L \bigl(material + (\alpha_{\text{cooler}} + \varepsilon_1^{1.209/V} + \varepsilon_3)^2 \bigr) 
                     + \alpha_{\text{cooler}} + 0.05071 
                   \Bigr] \\[10pt]
                &\quad + \dfrac{\eta\,\text{CR}\,G}{
                     0.03968\,T_a 
                ...
                &\quad + \Biggl[ 
                     \varepsilon_2 \left( 
                       -0.02623 \cdot \dfrac{A_{rc}\,T_a\,\varepsilon_4}{\sqrt{A_{cp}\,V}} 
                       - 1.966 
                     \right) 
                     - \dfrac{0.381\,\tau_1}{\varepsilon_2} 
                   \Biggr] 
                   \cdot \bigl( \tau_2\,(\varepsilon_1 + \tau_2) + \tau_4 \bigr) 
                   - 0.165
            \end{aligned}
            """)
            ...
            T_h &= T_c + \Biggl\lvert 
            \Biggl(
              \dfrac{A_{cp}\,\eta\,\text{CR}\,G}{
                1.645\,\dfrac{A_{cp}}{L} 
                + 2.261\,G L V \cdot A_{rc/cp}^{-0.2786^{17.843\,A_{cp}}}
              }
              + L\,\eta\,\text{CR}\,G 
              + 0.785\,\varepsilon_2 \tau_2 
                \sqrt{\tau_4 \sqrt{\dfrac{A_{cp}\,\varepsilon_4}{A N}}}
            \Biggr) 
            \\[12pt]
            &\quad \times \Biggl(
              -\sqrt{ 
                \dfrac{ A_{cp} \left(0.215 - 0.511 \dfrac{\alpha_{\text{cooler}}}{A_{rc/cp}}\right) }{ 
                  A (-A_{rc/cp} + N + material + 2.17) \sqrt{|material - 4.145|} 
                } 
              }
              + 0.340 
              + \dfrac{0.0364}{
                \left( -\dfrac{material}{\sqrt{\dfrac{A_{cp}}{A N}} + 0.954} \right)^{\!material}
              }
              + \dfrac{0.215}{\sqrt{|material - 4.154|}}
            \Biggr)
            \Biggr\rvert
            \end{aligned}
            """)
