...
A_rc = st.sidebar.number_input("A_rc (m²)", 1e-5, 0.1, 0.001, step=1e-6, format="%.6f")
A_cp = st.sidebar.number_input("A_cp (m²)", 1e-5, 0.2, 0.001, step=1e-6, format="%.6f")
G = st.sidebar.number_input("G (W/m²)", 0.0, 1500.0, 500.0, step=10.0)
V = st.sidebar.number_input("V (m/s)", 0.0, 10.0, 2.0, step=0.1)
T_a = st.sidebar.number_input("T_a (°k)", 273.0, 400.0, 293.0)
A = st.sidebar.number_input("Area A (m²)", 1e-8,1e-4, 1e-8, step=1e-7, format="%.9f")
...
alpha_cooler = st.sidebar.number_input("α_cooler", 0.01, 1.0, 0.02, step=0.01)
...
if is_all_features:
    ...
    taw_3 = st.sidebar.number_input("τ₃", 0.0, 1.0, 0.01, step=0.01)
    ...
    k_rc = st.sidebar.number_input("k_rc (W/m·K)", 0.1, 20.0, 5.0, step=1.0, format="%.6f")
    delta_Al = st.sidebar.number_input("Δ_Al (m)", 1e-6, 1.0, 1e-6, step=1e-6, format="%.6f")
    k_Al = st.sidebar.number_input("k_Al (W/m·K)", 50.0, 500.0, 200.0, step=1.0, format="%.6f")
...
if st.button("Prediction ", type="primary"):
    with st.spinner("Calculating..."):
        ...
        if is_eq:
            ...
            perf = calculate_performance(T_h_eq, T_c_eq)
            results["T_h Equation"] = (T_h_eq, None)
            results["T_c Equation"] = (None, T_c_eq)
            #power , current baraye equation
            c1, c2 = st.columns(2)
            with c1:
                st.metric("current (I)", f"{perf['I']:.5f} A")
                st.metric("voltage (V)", f"{perf['V_out']:.5f} V")
