...
A = st.sidebar.number_input("A (Area of legs) (m²)", 1e-8, 1e-4, 1e-6, step=1e-7, format="%.9f")
L = st.sidebar.number_input("L (Length of legs) (m)", 0.000001, 1e-1, 0.001, step=0.000001, format="%.6f")
optical = st.sidebar.number_input("$η_{optical}$ ", 0.01, 1.0, 0.9, step=0.01)
...
N = st.sidebar.number_input("N (Number of legs)", 1.0, 200.0, 127.0, step=1.0)
...
if is_all_features:
    ...
    k_Al = st.sidebar.number_input("$k_{Al}$ (W/m·K)", 50.0, 500.0, 300.0, step=1.0, format="%.6f")
        ...
        if is_eq:
            with st.expander("Equations"):
                if is_eq:
                   ...
                   \Bigr] \\[10pt]
                &\quad + \dfrac{\eta_{\text{optical}} \text{CR}\,G}{
                     0.03968\,T_a 
            ...
            T_h &= T_c + \Biggl\lvert 
            \Biggl(
              \dfrac{A_{cp}\,\eta_{\text{optical}}\text{CR}\,G}{
                1.645\,\dfrac{A_{cp}}{L} 
                + 2.261\,G L V \cdot A_{rc/cp}^{-0.2786^{17.843\,A_{cp}}}
              }
              + L\,\eta_{\text{optical}}\,\text{CR}\,G 
