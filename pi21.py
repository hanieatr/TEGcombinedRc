...
model_choice = st.sidebar.radio(
    ...
    [
        ...
        "Equation-Based (T_h Only)",
        "Equation-Based (T_c Only)"
    ]
)

is_all_features = "Complete Model" in model_choice
is_feature_selection = "Feature Selection" in model_choice
is_eq_th = "Equation-Based (T_h Only)" in model_choice
is_eq_tc = "Equation-Based (T_c Only)" in model_choice
...
if is_all_features:
    ...
    epsilon_2 = st.sidebar.number_input("ε₂ (T_c)", 0.01, 1.0, 0.9, step=0.01)
    ...
    taw_4 = st.sidebar.number_input("τ₄ (T_c)", 0.01, 1.0, 0.9, step=0.01)
...
if st.button("Prediction ", type="primary"):
    with st.spinner("Calculating..."):
        ...
        st.success("Done!")
        if is_eq_th or is_eq_tc:
            with st.expander("Equations"):
                if is_eq_th: st.latex(r"""
                    \begin{aligned}
